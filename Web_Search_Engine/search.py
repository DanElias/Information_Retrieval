...
def ranked_retrieval_search(query, total_docs, dictionary, ids_ranks):
    ...
    for token in unique_tokens:
        ...
        for docID_termF in token_postings_list:
            ...
            scores[doc_id] = scores[doc_id] + (doc_tf * query_tf * query_idf) + (ids_ranks[doc_id] * 1000) + title_relevance
...
def run_search(dict_file, postings_file, queries_file, results_file):
    ...
    with open(dict_file, "rb") as dictionary_f:
        ...
        ids_urls = pickle.load(dictionary_f)
        ids_ranks = pickle.load(dictionary_f)
    ...
    while line:
        ...
        for query in queries:
            if "\"" in query:
                ...
                for  new_query in split_queries:
        ...
        for query in expanded_queries:
            if(len(final_result_scores) == 0):
                final_result_scores = ranked_retrieval_search(query, total_docs, dictionary, ids_ranks)
            else: 
                final_result_scores = and_query(final_result_scores, ranked_retrieval_search(query, total_docs, dictionary, ids_ranks))
        ...
        for res in final_result:
            output_file.write(str(res) + ' ' + ids_urls[res] + ' ')
        #end with new line?
        output_file.write("\n")
